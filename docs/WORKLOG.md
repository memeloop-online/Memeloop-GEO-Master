...
- **未完成的一处验证**：`cargo clippy --workspace --all-targets` 仅剩 1 条 warning（`crates/domain/src/agent.rs` 的 `collapsible_if`），**未修**——该处编辑此前被用户拒绝，故保留待用户定夺，`-D warnings` 目前不通过。`crates/api` 内的同类 warning 已在本轮修掉。
